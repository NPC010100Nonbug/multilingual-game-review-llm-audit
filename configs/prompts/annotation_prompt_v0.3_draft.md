# 标注 Prompt v0.3（草稿，未冻结）— 感知不公平 / perceived unfairness

> **状态：DRAFT v0.3，未冻结。** 本文件是 `annotation_prompt_v0.2_draft.md` 的候选升级版；不得覆盖 v0.2 / v0.1，也不得据此改变冻结的 `codebook_version: v1.0`。
>
> **v0.3 相对 v0.2 的唯一改动：** 仅收紧 `evidence_span` 的格式合规——(a) 明确禁止用 `...`/`…`/省略号连接不连续片段，只允许 ` […] `；(b) 200 字上限改为可操作指令（原句过长时截取更短子串）；(c) 在输出前新增 evidence_span 自检清单。**A+B 判据、决策顺序、subtype/facet、标签枚举一律不动。** 这是 2026-08-20 冒烟测试中 2/5 条 `schema_invalid`（均为 evidence_span 违规、标签本身正确）触发的修订。
>
...
- `evidence_span` 仅在 `PRESENT` 填，且必须严格满足以下四条硬约束：
  ...
  2. **≤200 字：** 长度不超过 200 个 Unicode 字符。若能同时体现规范线索与系统归因的原句超过 200 字，**只截取仍能同时体现「不公平规范 + 系统归因」的最短子串**，不要整段照抄；宁可短而精准，也不要为凑全信息而超长。
...
3. **evidence_span（PRESENT 时最易出错，务必逐字核对）：**
   ...
   - 总长度是否 ≤200 字？若超了，是否已截成仍含「规范 + 系统归因」的更短子串？
...
- **目的：** v0.3 仅在 v0.2 基础上收紧 `evidence_span` 的格式合规（逐字、≤200 字可操作截取、禁止 `...` 只允许 ` […] `、新增输出前自检），以降低 `schema_invalid`（evidence_span 违规）比例。**不改动任何判据、决策顺序、标签/facet 语义。**（v0.2 的原有变化——澄清冲突指令、前置 `NA`/`ABSENT` 排他、明确 metadata 例外、逐字 evidence、可检查约束——一并继承。）
- **不可改变的边界：** 不把人工答案键或当前开发集的个案结论塞入正文；不用 gold 或 stress 修改本 prompt；不以更换模型代替 prompt 校准。
- **待验证（分两步，先小后大）：**
  1. **格式合格率冒烟（先做）：** 在同一 DeepSeek 条件下，对同样 5 条冒烟样本分别跑 v0.1、v0.2、v0.3，只看 `schema_invalid_rows` / evidence_span 违规是否随 v0.3 降到可接受水平。**格式合格率未稳住前，不烧完整 A/B。**
  2. **正式 A/B（合格后再做）：** 对 247 条已人工标好的开发评论运行同一模型条件下的候选版本，比较 JSON 可解析率、主标签混淆矩阵 / Cohen's kappa、PRESENT specific agreement、英文 PRESENT 召回、hard-negative 假阳性，以及 subtype/facet 一致率。不得只挑对某一版本有利的指标。
